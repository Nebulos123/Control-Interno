<script>
        ...
        // --- LÓGICA PARA EL FRASCO DE POROTOS --- (Sin cambios)
        ...
        // --- LÓGICA DE TOOLTIPS --- (Sin cambios, usa datos locales)
        ...
        // --- Bloque de funciones para gestionar tareas predeterminadas --- (Sin cambios)
        ...
        async function inicializarPanelesYDatos() { 
            ...
            if (esAdmin()) { 
                ...
                const { data, error } = await supabase.from('tareas_predeterminadas').select('id, descripcion'); 
        ...
        async function cargarVias() {
            ...
            try {
                const [viasRes, tareasRes] = await Promise.all([
                    supabase.from('vias').select(`id, numero_via, status, modelo_barrera`).order('numero_via', { ascending: true }),
                    supabase.from('novedades').select(`id, via_id, descripcion, created_at`).eq('resuelta', false)
                ]);
        ...
        function crearItemNovedad(novedad, esResuelta) { const item = document.createElement('li'); item.className = esResuelta ? 'novedad-resuelta' : 'novedad-pendiente'; item.dataset.novedadId = novedad.id; const fechaCreacion = new Date(novedad.created_at).toLocaleString('es-AR'); let infoAuditoria = ''; if (esResuelta && novedad.resolved_at && novedad.resolved_by) { const fechaResolucion = new Date(novedad.resolved_at).toLocaleString('es-AR'); infoAuditoria = `<span class="meta-info resuelta-info">Resuelta por ${novedad.resolved_by} el ${fechaResolucion}</span>`; } let botonesAccion = ''; const puedeEditar = esAdmin() || novedad.user_email === currentUserEmail; if (!esResuelta) { botonesAccion += `<button class="btn-resolver" title="Marcar como Resuelta">??</button>`; } if (puedeEditar) { botonesAccion += `<button class="btn-editar" title="Editar">??</button>`; } if (esAdmin()) { botonesAccion += `<button class="btn-eliminar" title="Eliminar">???</button>`; } item.innerHTML = `<div class="novedad-contenido"><span class="meta-info">Creada por ${novedad.user_email || 'Sistema'} el ${fechaCreacion}</span>${infoAuditoria}<p class="descripcion-texto">${novedad.descripcion}</p></div><div class="novedad-acciones">${botonesAccion}</div>`; return item; }
        ...
        async function manejarEliminarNovedad(novedadId) {
            ...
            if (confirm("¿Seguro que desea eliminar esta tarea/novedad?")) {
                try {
                    const { data: novedad, error: fetchError } = await supabase.from('novedades').select('via_id').eq('id', novedadId).single();
                    ...
                    const { error: deleteError } = await supabase.from('novedades').delete().eq('id', novedadId);
                    if (deleteError) throw deleteError;
                    mostrarNotificacion("Eliminado con éxito.");
                    const { count, error: countError } = await supabase.from('novedades').select('*', { count: 'exact', head: true }).eq('via_id', viaIdAfectada).eq('resuelta', false);
                    if (countError) throw countError;
                    if (count === 0) {
                        const { data: via, error: viaError } = await supabase.from('vias').select('status').eq('id', viaIdAfectada).single();
                        if(via && via.status !== 'normal') {
                            const { error: updateViaError } = await supabase.from('vias').update({ status: 'normal' }).eq('id', viaIdAfectada);
                            if (!updateViaError) {
